...
def printschedule(r):
    for d in range(len(r)/2):
        ...
        ret=flights[(destination,origin)][r[(2*d)+1]]          # Corrected for iterating wrongly
...
def hillclimb(domain,costf):
    ...
    while 1:
        ...
        for j in range(len(domain)):
            ...
            if (domain[j][0] == 0) and (domain[j][1] == 0): pass   # Workaround for when domain is (0,0)
            else:
...
""" Corrections to printschedule() for wrong iterations through list r
    Corrections to schedulecost() for wrong iterations through list r
    Correction to randomoptimize() was returning r instead of bestr
    Correction to mutate() in geneticoptimize()
    Corrected misspelled mutprod to mutprob
    Correction to mutate() so different step values can be added
    Corrected hillclimb for when domain[x]=(0,0) 
"""
